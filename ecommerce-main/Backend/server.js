...
app.post("/vendor_regitration_request", (req, res) => {
  ...
  const sql =
    "insert into Vendor_registration(`firstname`, `lastname`,`email`,`mobile`,`message`,`created_date`) values(?,?,?,?,?,?)";
...
app.get(`/vendor_request`, (req, res) => {
  const sql =
    "select * from `Vendor_registration` where deleted = 0 order by id desc";
  ...
  con.query(sql, [status, vendor_id], (err, data) => {
    if (err) {
      ...
      if (data.affectedRows == 1) {
        const insert =
          "insert into awt_vendor(`vendor_name`, `mobile`, `emailid`) values(?,?,?)";
...
app.post(`/add_return_order`, (req, res) => {
  ...
  con.query(getreturncount, (err, data) => {
    if (err) {
      ...
      const insertreturnorder =
        "insert into awt_return_exchange(`orderid`,`reason`,`user_id`,`return_no`,`return_date`,`created_date`) values(?,?,?,?,?,?)";

      con.query(
        insertreturnorder,
        [order_id, reasonid, user_id, returnno, currentDate, currentDate],
        (err, data) => {
          if (err) {
            return res.json(err);
          } else {
            ...
            const insertproduct =
              "insert into awt_return_cart(`orderid`,`return_id`,`proid`,`pname`,`price`,`pqty`,`cid`,`wishid`,`created_date`) values ?";
...
app.post("/getreturncartData", (req, res) => {
  ...
  const sql =
    "select * from `awt_return_cart` as arc left join `awt_productimg` as ap on ap.product_id = arc.proid where return_id = ? and ap.deleted = 0 group by arc.proid ";
...
app.post("/add_cancellation", (req, res) => {
  ...
  if (uid == undefined) {
    sql =
      "insert into awt_cancellation_reason(`title`,`description`,`created_by`,`created_date`) values(?,?,?,?)";
    param = [title, description, user_id, created_date];
  } else {
    sql =
      "update awt_cancellation_reason set title = ?, description = ?,updated_by = ?, updated_date = ? where id =?";
...
app.post("/add_seo", (req, res) => {
  ...
  if (u_id == undefined) {
    sql =
      "insert into awt_pages(`seo_title`,`seo_desc`,`created_date`,`created_by`) values(?,?,?,?)";
    param = [seotitle, seodescription, date, user_id];
  } else {
    sql =
      "update awt_pages set seo_title = ? , seo_desc = ? ,updated_date = ? , updated_by = ?  where id = ?";
...
app.post("/update_about", (req, res) => {
  ...
  const sql =
    "update `awt_about` set top_title = ? , top_desc = ? ,updated_date = ? where id = 1";
...
app.post("/add_producttag", (req, res) => {
  ...
  if (u_id == undefined) {
    sql =
      "insert into awt_producttag(`title`,`description`,`created_by`,`created_date`) values(?,?,?,?)";
    param = [title, description, user_id, created_date];
  } else {
    sql =
      "update awt_producttag set title = ? , description = ? , updated_by = ? ,updated_date = ? where id = ?";
    param = [title, description, user_id, created_date, u_id];
  }
...
app.post("/update_stock", (req, res) => {
  ...
  if (flag == "1") {
    const addstock =
      "insert into awt_productstockadd(`pro_id` , `stock`, `created_date`, `created_by`) values(?,?,?,?)";
    ...
    con.query(addstock, [pro_id, count, date, user_id], (err, data) => {
      if (err) {
        ...
        if (data.affectedRows == 1) {
          const getstock =
            "select * from awt_productstock where pro_id = ? and deleted = 0";
          ...
          con.query(getstock, [pro_id], (err, data) => {
            if (err) {
              ...
              if (data.length == 0) {
                const insert =
                  "insert into awt_productstock(`pro_id` , `stock`, `created_date`, `created_by`) values(?,?,?,?)";
                con.query(
                  insert,
                  [pro_id, count, date, user_id],
                  (err, data) => {
                    if (err) {
                      return res.json(err);
                    } else {
                ...
                const updatestock =
                  "update awt_productstock set stock = ? , updated_date = ? , updated_by = ? where pro_id = ?";

                con.query(
                  updatestock,
                  [newcount, date, user_id, pro_id],
                  (err, data) => {
                    if (err) {
                      return res.json(err);
                    } else {
    ...
    const addstock =
      "insert into awt_productstockremove(`pro_id` , `stock`, `created_date`, `created_by`) values(?,?,?,?)";
...
app.post("/add_SlotMaster", (req, res) => {
  ...
  const sql =
    "INSERT INTO SlotMaster (location, slot, title, created_by, created_date) VALUES (?, ?, ?, ?, ?)";
...
app.post("/update_SlotMaster", (req, res) => {
  ...
  const sql =
    "UPDATE SlotMaster SET location = ?, slot = ?, title = ?, updated_by = ?, updated_date = ? WHERE id = ?";
...
//   }
  ...
  const sql =
    "UPDATE awt_advertisements SET slot = ?, type = ?, title = ?, link = ?, target = ?,image = ?,iframe = ?,deleted = ? WHERE id = ?";
  const params = [
    slot,
    type,
    title,
...
app.post("/sendinquiry", upload10.single("image"), (req, res) => {
  ...
  con.query(
    sql,
    [user_id, product_id, inquiry, name, email, mobile, date, image],
    (err, data) => {
      if (err) {
      ...
      >
          ...
          <table style="width: 100%">
            <div>
              <tr style="height: 0">
                ...
                <td style="text-align: right">
                  <span style="font-size: 16px; line-height: 30px; color: #000000">11-oct-2024</span>
                </td>
              </tr>
            </div>
          </table>
        ...
        <main>
          ...
          >
            ...
            margin-bottom: 30px;"
              >
                Customization Request Pending
              </h1>
              <p style="margin: 0; margin-top: 17px; font-size: 16px; font-weight: 500;">
                Hi ${name},
              </p>
              <p style="margin: 0; margin-top: 17px; font-weight: 500; letter-spacing: 0.56px;">
                Your customization request has been received and is currently pending approval. We’ll notify you once it has been reviewed and approved.
              </p>
              <p style="margin-top: 40px; font-size: 18px; font-weight: 500; color: #1f1f1f;">
                Thank you for choosing Micasasucasa!
              </p>
